...
test('page scroll reads all roles before v6, and reverses continuously', () => {
  for (const viewport of [600, 1000]) {
    const start = storyAnchorVh(9.8, sequence) * viewport / 100;
    const travel = EXPERIENCE_SCROLL_VH * viewport / 100;
    for (const p of [0, .25, .75, 1, .75, .25, 0]) {
      ...
      assert.ok(Math.abs(scrollTime(state.videoY,state.base,4,sequence.end)-9.8) < 1e-9);
    }
    ...
    const finish = experienceScroll((sequence.scrollVh+EXPERIENCE_SCROLL_VH)*viewport/100,viewport,sequence);
